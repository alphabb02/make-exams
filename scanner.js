...
function onScannerSubjectChange() {
  const e=document.getElementById("scanner-subject-select").value,t=document.getElementById("scanner-model-select"),n=document.getElementById("scanner-student-card"),a=document.getElementById("scanner-model-info");
  if(t.innerHTML='<option value="">-- اختر النموذج --</option>',n.style.display="none",a.style.display="none",scannerState.selectedSubject=null,!e)return;
  let r=[],s="";
  if("__current__"===e)r=models,s=examCfg.subject;
  else {
    const t=loadProjectFromStorage(parseInt(e));
    t&&(r=t.models||[],s=t.examCfg?.subject||"غير محدد")
  }
  if(0===r.length)return void showToast("لا توجد نماذج لهذه المادة","error");
  scannerState.selectedSubject= {
    val:e,models:r,name:s
  },r.forEach((e,n)=> {
    const a=document.createElement("option");
    a.value=n,a.textContent=`${e.name} — ${e.questions.length} سؤال`,t.appendChild(a)
  });
  const o=document.createElement("option");
  o.value="auto",o.textContent="🔍 تحديد تلقائي (من الورقة)",t.insertBefore(o,t.children[1]),n.style.display="block",document.getElementById("results-subject-label").textContent=s
}
function onScannerModelChange() {
  ...
  const n=parseInt(e),a=scannerState.selectedSubject.models[n];
  scannerState.selectedModelIdx=n,scannerState.selectedModel=a,t.style.display="block",t.innerHTML=`✅ النموذج: <strong style="color:var(--accent);">${a.name}</strong> | عدد الأسئلة: <strong>${a.questions.length}</strong> | مفتاح الإجابات جاهز للمقارنة`
}
...
function processOMRImage(e,t,n) {
  const a=scannerState.selectedModel||scannerState.selectedSubject?.models[0];
  a?(document.getElementById("scan-result-content").innerHTML='<div style="color:var(--accent);">⏳ جارٍ تحليل الصورة...</div>',setTimeout(()=> {
    try {
      const r=analyzeOMRBubbles(e,t,n,a.questions.length);
      scannerState.currentScannedAnswers=r;
      const s=[];
      Object.entries(r).forEach(([e,t])=> {
        null!==t&&"MULTI"!==t||s.push(parseInt(e))
      }),scannerState.pendingManualQuestions=s,showScanResults(r,a),s.length>0?showManualOverride(s,a):computeAndShowScore(r,a)
    }
    catch(e) {
...
function analyzeOMRBubbles(e,t,n,a) {
  const r=e.data,s= {

  },o=new Uint8Array(t*n);
  for(let e=0; e<t*n; e++) {
    const t=r[4*e],n=r[4*e+1],a=r[4*e+2];
    o[e]=Math.round(.299*t+.587*n+.114*a)
  }
  const l=new Float32Array(n);
  for(let e=0; e<n; e++) {
    let n=0;
    for(let a=0; a<t; a++)o[e*t+a]<80&&n++;
    l[e]=n/t
  }
  const c=new Float32Array(t);
  for(let e=0; e<t; e++) {
    let a=0;
    for(let r=0; r<n; r++)o[r*t+e]<80&&a++;
    c[e]=a/n
  }
  let d=Math.floor(.05*n),u=Math.floor(.95*n);
  ...
  let i=Math.floor(.02*t),m=Math.floor(.98*t);
  for(let e=Math.floor(.02*t); e<Math.floor(.98*t); e++)if(c[e]>=.08) {
    i=e;
    break
  }
  for(let e=Math.floor(.98*t); e>i; e--)if(c[e]>=.08) {
    m=e;
    break
  }
  const g=(u-d)/5,f=(m-i)/(a+1),S=["A","B","C","D"];
  for(let e=0; e<a; e++) {
    const a=e+1,r=Math.floor(i+a*f),l=Math.floor(i+(a+1)*f),c=[];
    for(let e=0; e<4; e++) {
      const a=e+1,s=Math.floor(d+a*g),u=Math.floor(d+(a+1)*g),i=Math.max(2,Math.floor(.2*g)),m=Math.max(2,Math.floor(.2*f));
      let S=0,p=0;
      for(let e=s+i; e<u-i; e++)for(let a=r+m; a<l-m; a++)a>=0&&a<t&&e>=0&&e<n&&(o[e*t+a]<110&&S++,p++);
      c.push(p>0?S/p:0)
    }
    const u=c.map((e,t)=>( {
      label:S[t],ratio:e
    })).filter(e=>e.ratio>=.15);
    if(0===u.length)s[e+1]=null;
    else if(1===u.length)s[e+1]=u[0].label;
    else {
      u.sort((e,t)=>t.ratio-e.ratio);
      const t=u[0].ratio-u[1].ratio;
      s[e+1]=t<.07?"MULTI":u[0].label
    }
  }
  return s
}
function showScanResults(e,t) {
  const n=t.questions.length,a=Object.values(e).filter(e=>e&&"MULTI"!==e).length,r=Object.values(e).filter(e=>null===e||"MULTI"===e).length;
  let s=`<div style="margin-bottom:10px;font-size:12px;color:var(--text2);">\n    تم اكتشاف <strong style="color:var(--accent3);">${a}</strong> إجابة من أصل ${n}\n    ${r>0?`| <strong style="color:var(--warn);">$ {
    r
  }
  </strong> غير واضحة`:""}\n  </div>`;
  s+='<div style="display:grid;grid-template-columns:repeat(5,1fr);gap:4px;">';
  for(let t=1; t<=n; t++) {
    const n=e[t];
    let a="var(--bg)",r="var(--text2)",o=n||"?";
    null===n&&(a="#ffb84f22",r="var(--warn)",o="؟"),"MULTI"===n&&(a="#ff4f6a22",r="var(--danger)",o="!!"),n&&"MULTI"!==n&&(a="#4f7cff22",r="var(--accent)"),s+=`<div style="padding:4px;background:${a};border-radius:6px;text-align:center;border:1px solid ${a};">\n      <div style="font-size:9px;color:var(--text3);">${t}</div>\n      <div style="font-size:13px;font-weight:700;color:${r};">${o}</div>\n    </div>`
  }
  s+="</div>",document.getElementById("scan-result-content").innerHTML=s
}
function showManualOverride(e,t) {
  ...
  document.getElementById("manual-override-list").innerHTML=e.map(e=> {
    const n=t.questions[e-1],a=n?n.choices.map(e=>e.label):["A","B","C","D"],r="MULTI"===scannerState.currentScannedAnswers[e];
...
function computeAndShowScore(e,t) {
  let n=0;
  const a=t.questions.length;
  t.questions.forEach((t,a)=> {
    const r=e[a+1];
    r&&r===t.correctLabel&&n++
  });
  const r=document.getElementById("score-summary-panel");
  document.getElementById("score-display").textContent=`${n} / ${a}`,document.getElementById("score-percent").textContent=`${Math.round(n/a*100)}%`,r.style.display="block",scannerState._pendingResult= {
    studentId:scannerState.currentStudentId,modelName:t.name,modelLetter:t.letter,subject:scannerState.selectedSubject?.name||"",answers: {
      ...e
    },correct:n,total:a,score:n,percent:Math.round(n/a*100),timestamp:(new Date).toISOString()
  }
}
...
function renderResultsTable() {
  ...
  if(!t)return;
  const a=scannerState.selectedSubject?.name||"",r=a?scannerState.allResults.filter(e=>e.subject===a):scannerState.allResults;
  if(0===r.length)return void(e.style.display="none");
  e.style.display="block",t.innerHTML=r.map((e,t)=> {
    const n=e.percent??Math.round(e.correct/e.total*100),a=n>=60?"var(--accent3)":n>=50?"var(--warn)":"var(--danger)";
  ...
  const s=r.length>0?Math.round(r.reduce((e,t)=>e+(t.percent??0),0)/r.length):0,o=r.filter(e=>(e.percent??0)>=60).length;
  n.innerHTML=`إجمالي الطلاب: <strong>${r.length}</strong> | متوسط الدرجات: <strong style="color:var(--accent);">${s}%</strong> | الناجحون (≥60%): <strong style="color:var(--accent3);">${o}</strong>`
}
...
function exportResultsExcel() {
  ...
  const n=XLSX.utils.book_new(),a=[["الرقم الجامعي","النموذج","المادة","الإجابات الصحيحة","مجموع الأسئلة","العلامة","النسبة المئوية","تاريخ المسح"]];
  t.forEach(e=> {
    const t=e.percent??Math.round(e.correct/e.total*100);
    a.push([e.studentId,e.modelName||e.modelLetter||"",e.subject,e.correct,e.total,e.score,t+"%",e.timestamp?new Date(e.timestamp).toLocaleString("ar-EG"):""])
  });
  const r=XLSX.utils.aoa_to_sheet(a);
  r["!cols"]=[ {
    wch:18
  }, {
    wch:14
  ...
  const s=`نتائج_${e||"الامتحان"}_${(new Date).toLocaleDateString("ar-EG").replace(/\//g,"-")}.xlsx`;
  XLSX.writeFile(n,s),showToast("تم تحميل ملف Excel ✓","success")
}