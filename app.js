...
<body dir="${dir}"><div class="Section1">
  ...
  models.forEach(m => {
    ...
    if (opts.answerTable) {
      ...
      if (isOMR) {
        // ── OMR circles table for Word export ──
        ...
        for (let tableStart = 0; tableStart < m.questions.length; tableStart += perRow) {
          ...
          const numCols = tableQuestions.length;
          let tHtml = `<table dir="ltr" style="border-collapse:collapse;border:1pt solid #666;margin-bottom:8pt;direction:ltr;">`;
          // Question numbers row
          tHtml += `<tr><td style="border:1pt solid #666;padding:2pt 4pt;font-size:7pt;font-weight:700;background:#e5e5e5;white-space:nowrap;min-width:22pt;">Q</td>`;
          for (let i = 0; i < numCols; i++) {
            tHtml += `<td style="border:1pt solid #666;padding:2pt 1pt;font-size:6.5pt;font-weight:700;text-align:center;background:#f0f0f0;min-width:18pt;">${tableStart + i + 1}</td>`;
          }
          tHtml += `</tr>`;
          // Answer label rows with circles (○)
          choiceLabels.slice(0, 4).forEach(label => {
            tHtml += `<tr><td style="border:1pt solid #666;padding:2pt 4pt;font-size:7pt;font-weight:700;background:#e5e5e5;white-space:nowrap;">${label}</td>`;
            for (let i = 0; i < numCols; i++) {
              tHtml += `<td style="border:1pt solid #666;padding:2pt 1pt;text-align:center;font-size:10pt;line-height:1;">&#9711;</td>`;
            }
            tHtml += `</tr>`;
          });
          tHtml += `</table>`;
        ...
        const omrLabel = `<p style="font-size:8pt;color:#374151;margin-bottom:4pt;"><strong>ورقة الإجابات:</strong> ضع دائرة حول الإجابة الصحيحة</p>`;
