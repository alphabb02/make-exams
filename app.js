...
<body dir="${dir}"><div class="Section1">
  ...
  models.forEach(m => {
    ...
    if (opts.answerTable) {
      ...
      if (isOMR) {
        // ── OMR circles table for Word export — always 20 cols, full A4 width ──
        const choiceLabels = m.questions[0]?.choices?.map(c => c.label) || ['A','B','C','D'];
        const LABEL_W = 10;    // mm
        const PAGE_W  = 170;   // mm usable (A4 - margins)
        const COLS    = perRow; // always 20
        const cellW   = ((PAGE_W - LABEL_W) / COLS).toFixed(1);
        let omrTables = '';

        for (let tableStart = 0; tableStart < m.questions.length; tableStart += COLS) {
          const rowEnd = tableStart + COLS; // always render 20 cols
          ...
          for (let i = tableStart; i < rowEnd; i++) {
            const hasQ = i < m.questions.length;
            tHtml += `<td style="border:1pt solid #555;padding:3pt 1pt;font-size:7pt;font-weight:700;text-align:center;background:#e8eaf0;width:${cellW}mm;">${hasQ ? i + 1 : ''}</td>`;
          }
          tHtml += `</tr>`;

          // ── Answer label rows with circles — empty cell if no question ──
          choiceLabels.slice(0, 4).forEach(label => {
            ...
            for (let i = tableStart; i < rowEnd; i++) {
              const hasQ = i < m.questions.length;
              tHtml += `<td style="border:1pt solid #555;padding:4pt 1pt;text-align:center;font-size:11pt;line-height:1;height:16pt;">${hasQ ? '&#9711;' : ''}</td>`;
            }
            tHtml += `</tr>`;
          });
