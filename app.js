...
function buildOMRTableRows(totalQuestions, choiceLabels, mode) {
  ...
  for (let start = 0; start < totalQuestions; start += COLS) {
    const rowCount = Math.min(COLS, totalQuestions - start);
    ...
    for (let c = 0; c < rowCount; c++) {
      ...
      t += `<td style="border:${border};padding:3px 1px;font-size:${numFs};font-weight:700;`
         + `text-align:center;background:${numBg};${cW}">${qNum}</td>`;
    }
    t += `</tr>`;
      ...
      for (let c = 0; c < rowCount; c++) {
        if (isWord) {
          t += `<td style="border:${border};padding:${padding};text-align:center;`
             + `font-size:${circleFs};line-height:1;${cellH}">&#9711;</td>`;
        } else {
...
${mathJaxScript()}
  ...
  // Group questions by 20 per table — only actual questions, no empty padding
  const tables = [];
  const firstQ = model.questions[0];
  ...
  for (let tableStart = 0; tableStart < model.questions.length; tableStart += 20) {
    const tableQuestions = model.questions.slice(tableStart, tableStart + 20);
    ...
    // Choice rows with filled/empty circles
