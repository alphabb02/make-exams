...
// ══════════════════════════════════════════
// SHARED OMR TABLE BUILDER
// Always renders exactly COLS=20 columns per row.
// Cells beyond totalQuestions are fully empty (no number, no circle).
// Applies to HTML preview, PDF export, and Word export.
// ══════════════════════════════════════════
function buildOMRTableRows(totalQuestions, choiceLabels, mode) {
  const COLS       = 20;
  const isWord     = mode === 'word';

  // Word uses mm for widths; HTML/PDF use % to fill their container
  const LABEL_W_MM = 10, PAGE_W_MM = 170;
  const cellWMM    = ((PAGE_W_MM - LABEL_W_MM) / COLS).toFixed(1);
  const labelPct   = '4%';
  const cellPct    = (96 / COLS).toFixed(2) + '%';

  const labelBg  = isWord ? '#d0d4de' : '#e5e5e5';
  const numBg    = isWord ? '#e8eaf0' : '#f0f0f0';
  const border   = isWord ? '1pt solid #555' : '1px solid #666';
  const numFs    = isWord ? '7pt'   : '6.5pt';
  const lblFs    = isWord ? '7.5pt' : '7pt';
  const circleFs = isWord ? '11pt'  : '10pt';
  const cellH    = isWord ? 'height:16pt;' : 'height:16px;';
  const padding  = isWord ? '4pt 1pt' : '2px 1px';
  const lblPad   = isWord ? '4pt 4pt' : '3px 4px';
  const mb       = isWord ? '6pt' : '8px';

  // table-layout:fixed ensures all 20 data cols share width equally
  const tableStyle = isWord
    ? `style="border-collapse:collapse;border:${border};margin-bottom:${mb};direction:ltr;width:100%;table-layout:fixed;"`
    : `style="width:100%;border-collapse:collapse;border:1px solid #666;margin-bottom:${mb};table-layout:fixed;direction:ltr;"`;

  let tables = '';
  for (let start = 0; start < totalQuestions; start += COLS) {
    let t = `<table ${tableStyle}>`;

    // ── Q number row ──
    t += `<tr>`;
    const lW = isWord ? `width:${LABEL_W_MM}mm;` : `width:${labelPct};`;
    t += `<td style="border:${border};padding:${lblPad};font-size:${lblFs};font-weight:700;`
       + `background:${labelBg};text-align:center;white-space:nowrap;${lW}">Q</td>`;
    for (let c = 0; c < COLS; c++) {
      const qNum = start + c + 1;
      const hasQ = qNum <= totalQuestions;
      const cW   = isWord ? `width:${cellWMM}mm;` : `width:${cellPct};`;
      t += `<td style="border:${border};padding:3px 1px;font-size:${numFs};font-weight:700;`
         + `text-align:center;background:${numBg};${cW}">${hasQ ? qNum : ''}</td>`;
    }
    t += `</tr>`;
    ...
    choiceLabels.slice(0, 4).forEach(label => {
      t += `<tr>`;
      t += `<td style="border:${border};padding:${lblPad};font-size:${lblFs};font-weight:700;`
         + `background:${labelBg};text-align:center;white-space:nowrap;">${label}</td>`;
      for (let c = 0; c < COLS; c++) {
        const qNum = start + c + 1;
        const hasQ = qNum <= totalQuestions;
        if (isWord) {
          t += `<td style="border:${border};padding:${padding};text-align:center;`
             + `font-size:${circleFs};line-height:1;${cellH}">${hasQ ? '&#9711;' : ''}</td>`;
        } else {
          t += `<td style="border:${border};padding:2px 1px;text-align:center;${cellH}">`
             + (hasQ
               ? `<div style="display:inline-block;width:11px;height:11px;border:1.2px solid #333;border-radius:50%;background:#fff;"></div>`
               : '')
             + `</td>`;
        }
      }
...
function renderExamPaper(model, idx) {
  ...
  if (isOMR && opts.answerTable) {
    ...
    const tablesHtml = buildOMRTableRows(model.questions.length, answerLabels, 'html');
    omrGrid = `<div style="padding:8px 12px;background:#f3f4f6;border:1px solid #d1d5db;border-radius:6px;margin-bottom:1rem;font-size:${fs-2}px;color:#374151;line-height:1.5;">
      <strong>ورقة الإجابات:</strong> ضع دائرة حول الإجابة الصحيحة
    </div><div style="width:100%;overflow-x:auto;">${tablesHtml}</div>`;
    answerTable = '';
  } else {
...
function buildQuestionPaperHtml(model) {
  ...
  if (isOMR && opts.answerTable) {
    const answerLabels = model.questions[0].choices.map(c => c.label) || ['A','B','C','D'];
    const tablesHtml = buildOMRTableRows(model.questions.length, answerLabels, 'pdf');
    omrGrid = `<div style="padding:7px 10px;background:#f3f4f6;border:1px solid #d1d5db;border-radius:4px;margin-bottom:10pt;font-size:8pt;color:#374151;line-height:1.4;">
      <strong>ورقة الإجابات:</strong> ضع دائرة حول الإجابة الصحيحة
    </div>${tablesHtml}`;
  } else {
...
${mathJaxScript()}
  ...
  // Group questions by 20 per table — always exactly 20 cols, empty cells for missing questions
  const OMRCOLS = 20;
  const tables = [];
  const firstQ0 = model.questions[0];
  const choiceLabelsAll = firstQ0 ? firstQ0.choices.map(c => c.label) : ['A','B','C','D'];

  for (let tableStart = 0; tableStart < model.questions.length; tableStart += OMRCOLS) {
    let tableHtml = `<table class="omr-table">`;

    // Row 1: Question numbers — always 20 cols, blank if no question
    tableHtml += `<tr><td class="row-label">السؤال</td>`;
    for (let c = 0; c < OMRCOLS; c++) {
      const qNum = tableStart + c + 1;
      const hasQ = qNum <= model.questions.length;
      tableHtml += `<td class="qnum-cell">${hasQ ? qNum : ''}</td>`;
    }
    tableHtml += `</tr>`;

    // Choice rows — circle if question exists, empty cell if not
    choiceLabelsAll.forEach((rowLabel, idx) => {
      tableHtml += `<tr><td class="row-label">${rowLabel}</td>`;
      for (let c = 0; c < OMRCOLS; c++) {
        const qNum     = tableStart + c + 1;
        const hasQ     = qNum <= model.questions.length;
        const question = hasQ ? model.questions[qNum - 1] : null;
        if (!hasQ) {
          tableHtml += `<td style="padding:8px 4px;border:1px solid #333;"></td>`;
        } else {
...
<body dir="${dir}"><div class="Section1">
  ...
  models.forEach(m => {
    ...
    if (opts.answerTable) {
      ...
      if (isOMR) {
        // ── OMR circles table for Word — uses shared buildOMRTableRows (always 20 cols) ──
        const choiceLabels = m.questions[0]?.choices?.map(c => c.label) || ['A','B','C','D'];
        const omrTables = buildOMRTableRows(m.questions.length, choiceLabels, 'word');
