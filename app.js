...
function renderExamPaper(model, idx) {
  ...
  if (isOMR && opts.answerTable) {
    ...
    const tables = [];
    const _answerLabels = model.questions[0].choices.map(c => c.label) || ['A', 'B', 'C', 'D'];
    for (let tableStart = 0; tableStart < model.questions.length; tableStart += 20) {
      const numCols = Math.min(20, model.questions.length - tableStart);

      let tableHtml = `<table class="omr-table" style="width:100%;table-layout:fixed;">`;

      // Row 1: Question numbers — always 20 cols
      tableHtml += `<tr><td class="row-label">Q</td>`;
      for (let i = 0; i < 20; i++) {
        const hasQ = i < numCols;
        tableHtml += hasQ
          ? `<td class="qnum-cell">${tableStart + i + 1}</td>`
          : `<td class="qnum-cell" style="background:#f5f5f5;"></td>`;
      }
      tableHtml += `</tr>`;

      // Choice rows — always 20 cols
      _answerLabels.slice(0, 4).forEach((label) => {
        tableHtml += `<tr><td class="row-label">${label}</td>`;
        for (let i = 0; i < 20; i++) {
          const hasQ = i < numCols;
          tableHtml += hasQ
            ? `<td style="padding:1px;"><div class="omr-circle"></div></td>`
...
function buildAnswerFillTable(count, hBg, fs) {
  ...
  for (let r = 0; r < count; r += perRow) {
    const rowCount = Math.min(perRow, count - r);
    const nums   = [];
    const blanks = [];
    for (let i = 0; i < perRow; i++) {
      const hasQ = i < rowCount;
      nums.push(hasQ
        ? `<td style="text-align:center;border:1px solid #c8d0e0;padding:2px 1px;min-width:28px;font-size:6.5pt;font-weight:700;color:#333;">${r + i + 1}</td>`
        : `<td style="border:1px solid #c8d0e0;padding:2px 1px;min-width:28px;background:#f5f5f5;"></td>`);
      blanks.push(hasQ
        ? `<td style="border:1px solid #c8d0e0;padding:0;min-width:28px;height:16px;"></td>`
        : `<td style="border:1px solid #c8d0e0;padding:0;min-width:28px;height:16px;background:#f5f5f5;"></td>`);
    }
  ...
  return `
    <div style="margin-top:1.5rem;page-break-inside:avoid;direction:ltr;">
      <table style="border-collapse:collapse;border:1px solid #c8d0e0;table-layout:fixed;width:100%;">${tableRows}</table>
    </div>`;
}
  ...
  if (isOMR && opts.answerTable) {
    ...
    if (opts.answerTable) {
      ...
      for (let r = 0; r < model.questions.length; r += perRow) {
        const rowCount = Math.min(perRow, model.questions.length - r);
        const nums = [], blanks = [];
        for (let i = 0; i < perRow; i++) {
          const hasQ = i < rowCount;
          nums.push(hasQ
            ...
            : `<td style="border:1px solid #c8d0e0;padding:2px 1px;min-width:28px;background:#f5f5f5;"></td>`);
          blanks.push(hasQ
            ? `<td style="border:1px solid #c8d0e0;min-width:28px;height:16px;"></td>`
            : `<td style="border:1px solid #c8d0e0;min-width:28px;height:16px;background:#f5f5f5;"></td>`);
        }
        tRows += `<tr>
          <td style="border:1px solid #c8d0e0;padding:2px 4px;font-size:6.5pt;font-weight:700;
      ...
      fillTableHtml = `<div style="margin-top:14pt;page-break-inside:avoid;direction:ltr;">
        <table style="border-collapse:collapse;border:1px solid #c8d0e0;table-layout:fixed;width:100%;">${tRows}</table>
      </div>`;
    }
  }
...
${mathJaxScript()}
  ...
  const tables = [];
  const _sheetLabels = model.questions[0] ? model.questions[0].choices.map(c => c.label) : ['A','B','C','D'];
  for (let tableStart = 0; tableStart < model.questions.length; tableStart += 20) {
    const numCols = Math.min(20, model.questions.length - tableStart);
    ...
    // Choice rows — always 20 cols
    _sheetLabels.forEach((rowLabel, idx) => {
      tableHtml += `<tr><td class="row-label">${rowLabel}</td>`;
      for (let i = 0; i < 20; i++) {
        const hasQ = i < numCols;
        if (!hasQ) {
          tableHtml += `<td style="padding:8px 4px;background:#f5f5f5;"></td>`;
        } else {
          const question  = model.questions[tableStart + i];
...
<body dir="${dir}"><div class="Section1">
  ...
  models.forEach(m => {
    ...
    if (opts.answerTable) {
      ...
      const isOMRWord = design.answerStyle === 'highlighted';

      if (isOMRWord) {
        // OMR: always 20 cols, empty cells for missing questions
        const omrLabels = m.questions[0]?.choices?.map(c => c.label) || ['A','B','C','D'];
        const LABEL_W = 10; const PAGE_W = 170;
        const cellW = ((PAGE_W - LABEL_W) / perRow).toFixed(1);
        let omrTables = '';
        for (let r = 0; r < m.questions.length; r += perRow) {
          const rowCount = Math.min(perRow, m.questions.length - r);
          let t = `<table dir="ltr" style="border-collapse:collapse;border:1pt solid #555;margin-bottom:6pt;direction:ltr;width:100%;table-layout:fixed;">`;
          // Q numbers row
          t += `<tr><td style="border:1pt solid #555;padding:3pt 4pt;font-size:7.5pt;font-weight:700;background:#d0d4de;text-align:center;width:${LABEL_W}mm;">Q</td>`;
          for (let i = 0; i < perRow; i++) {
            const hasQ = i < rowCount;
            t += `<td style="border:1pt solid #555;padding:3pt 1pt;font-size:7pt;font-weight:700;text-align:center;background:${hasQ ? '#e8eaf0' : '#f5f5f5'};width:${cellW}mm;">${hasQ ? r + i + 1 : ''}</td>`;
          }
          t += `</tr>`;
          // Choice rows
          omrLabels.slice(0, 4).forEach(label => {
            t += `<tr><td style="border:1pt solid #555;padding:4pt 4pt;font-size:7.5pt;font-weight:700;background:#d0d4de;text-align:center;">${label}</td>`;
            for (let i = 0; i < perRow; i++) {
              const hasQ = i < rowCount;
              t += hasQ
                ? `<td style="border:1pt solid #555;padding:4pt 1pt;text-align:center;font-size:11pt;line-height:1;height:16pt;">&#9711;</td>`
                : `<td style="border:1pt solid #555;background:#f5f5f5;"></td>`;
            }
            t += `</tr>`;
          });
          t += `</table>`;
        ...
        // Standard fill table — always 20 cols, empty cells for missing questions
        let tRows = '';
        for (let r = 0; r < m.questions.length; r += perRow) {
          const rowCount = Math.min(perRow, m.questions.length - r);
          const nums = [], blanks = [];
          for (let i = 0; i < perRow; i++) {
            const hasQ = i < rowCount;
            nums.push(hasQ
              ? `<td style="text-align:center;border:1pt solid #c8d0e0;padding:2pt;width:20pt;font-size:7pt;font-weight:700;">${r + i + 1}</td>`
              : `<td style="border:1pt solid #c8d0e0;width:20pt;background:#f5f5f5;"></td>`);
            blanks.push(hasQ
              ? `<td style="border:1pt solid #c8d0e0;width:20pt;height:12pt;"></td>`
        ...
        fillTable = `<br><table dir="ltr" style="border-collapse:collapse;direction:ltr;table-layout:fixed;width:100%;">${tRows}</table>`;
      }
