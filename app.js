...
function saveProjectToStorage(projectData) {
  try {
    ...
    const project = {
      ...
      name: projectData.examCfg?.subject || 'مشروع بدون اسم',
    ...
    console.log(`Project saved: ${project.questions} questions, ${project.models} models`);
    return project.id;
  } catch (e) {
...
function autoSaveCurrentProject() {
  ...
  console.log('Auto-saving project with:', {
    questions: questions.length,
    models: models.length
  });
  const projectData = {
    id: window.currentProjectId || null,
    questions: questions,
    colMap: colMap,
    opts: opts,
    design: design,
...
${mathJaxScript()}
  ...
  for (let tableStart = 0; tableStart < model.questions.length; tableStart += 20) {
    ...
    options.forEach(opt => {
      tableHtml += `<tr><td class="row-label">${opt.label}</td>`;
      for (let i = 0; i < numCols; i++) {
        const question = tableQuestions[i];
        const choices = question.choices || [];
        
        // Get the choice for this option (e.g., if opt.label = 'ب', get second choice)
        const choice = choices[opt.index];
...
function loadProject(id) {
  ...
  if (project) {
    console.log('Project loaded:', {
      questions: project.questions?.length || 0,
    ...
    console.log('Models variable:', models.length, 'models loaded');
    ...
    // إذا كانت هناك نماذج، انتقل لعرضها
    if (models.length > 0) {
      console.log('Rendering models...');
      goPanel(3);
      // تأكد من أن الـ DOM جاهز قبل render
      setTimeout(() => {
        console.log('Calling renderModelTabs with', models.length, 'models');
        renderModelTabs();
        showToast(`تم تحميل ${models.length} نموذج ✓`, 'success');
      }, 100);
    } else {
      goPanel(0);
